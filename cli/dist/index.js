...
async function pickOption(message, options) {
    if (options.length === 0)
        throw new Error(`No available options for "${message}"`);
...
async function promptCredentialFields(fields) {
    const result = {};
...
async function main() {
    ...
    const frontend = await pickOption('Select a frontend framework:', manifest.frontends.filter(f => f.available));
    const backend = await pickOption('Select a backend framework:', manifest.backends.filter(b => b.available));
    const dbChoice = await pickOption('Select a database:', manifest.databases.filter(d => d.available));
